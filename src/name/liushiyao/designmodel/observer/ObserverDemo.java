 ...
 *
 * 观察者模式 1. 抽象主题（Subject）模式 主题角色吧所有的观察者对象的引用保存在一个列表中，每个主题都可以有任何数量的观察者。主题 提供一个接口，可以加上或者撤销观察者对象。
 * ----可以使用一个抽象类或者一个接口来实现，在具体情况下可以使用具体类实现 2. 抽象观察者（Observer）模式 为所有的具体观察者定义一个接口，在得到通知时通知更新自己。
 * ----可以使用一个抽象类或者一个接口来实现，在具体情况下可以使用具体类实现
 *
 * 3. 具体主题（ConcreteSubject）模式 具体主题保存对具体对象观察者对象有用的内部状态，在这种内部状态改变时，给其观察者发送一个通知。 4.
 * 具体观察者(ConcreteObserver)模式 具体观察者角色用于保存一个指向具体主题对象的引用，和一个与主题的状态相符的状态。具体观察者角色
 ...
 * 观察者模式
 *
 *    含义：定义对象间一种一对多的依赖关系， 使得每当一个对象发生改变时，所有的依赖于它的对象都会得到通知并且自动更新。
 *
 *      1. 抽象主题（Subject）模式
 ...
 *
 *      使用步骤：被观察者类继承Observable,使用父类的setChanged方法和notifyObservers方法，传递通知参数
 *            观察者类实现Observer中的update方法，用于接收被观察者发送过来的通知信息。
 *            使用时，为被观察者添加观察对象
 */
